...
def enviar():
    ...
    foto_poste = request.files.getlist('foto_poste[]')
...
def salvar_no_banco(cidade, logradouro, numero, bairro, barramento, ocupante,
                    ...
                    irregularidades, foto_ocupante, foto_poste,
...
def admin():
    try:
        ...
        cursor.execute("""
        SELECT cidade, logradouro, numero, bairro, barramento, ocupante, nivel_fixacao, tipo_cabo, equipamento, placa_identificacao
        , irregularidades, foto_ocupante, foto_poste, latitude, longitude, utm_x, utm_y, utm_zone_number, utm_zone_letter, data_hora 
        FROM sch_app_fiscal.tb_reg_app_fiscal ORDER BY DATA_HORA DESC 
        """)
        tb_reg_app_fiscal = cursor.fetchall()
        print(tb_reg_app_fiscal)
        cursor.close()
        conexao.close()
    except mysql.connector.Error as err:
        tb_reg_app_fiscal = []
    ...
    return render_template('admin.html', registros=tb_reg_app_fiscal)
